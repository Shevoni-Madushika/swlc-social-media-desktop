...
class RegisterControllerTest extends ApplicationTest {
    ...
    void testValidationFailure() {
        Platform.runLater(() -> {
            when(channelname.getText()).thenReturn(null);
            when(password.getText()).thenReturn(null);

            registerController.btnregisterOnAction(mock(ActionEvent.class));
    ...
    void testSuccessfulRegistration() throws IOException {
            when(channelname.getText()).thenReturn("kushan");
        when(password.getText()).thenReturn("1234");
        when(mockFile.getName()).thenReturn("Screenshot (476).png");
        registerController.selectedFile = mockFile;

        when(mockChannelModel.registerChannel(any(ChannelDTO.class), eq(mockFile)))
                .thenReturn(new ChannelDTO("kushan", "1234", "Screenshot (476).png"));
    ...
    void testRegistrationFailure() {
        Platform.runLater(() -> {
            when(channelname.getText()).thenReturn("TestChannel");
